...
export declare interface NodeCoreBase {
  ...
  on(
    event: "shutdown",
    listener: (eventArgs: { error: Error; restart: boolean; cancel(): void; isCancelled(): boolean }) => void
  ): this;
  on(
    event: "packet",
    listener: (eventArgs: {
  ...
  on(
    event: "client-init",
    listener: (eventArgs: { payload: PayloadLike[]; cancel(): void; isCancelled(): boolean }) => void
  ): this;
  on(event: "plugins", listener: (eventArgs: { plugins: NodeCorePlugin[] }) => void): this;
  on(
    event: "pipe.pre",
    listener: (eventArgs: { name: string; guid: Guid; plugin: Guid; cancel(): void; isCancelled(): boolean }) => void
  ): this;
  ...
  // Core Plugin events
  on(
    event: "core.initialize",
    listener: (eventArgs: {
      values: { osName: string; filename: string; isAdmin: boolean };
      cancel(): void;
      isCancelled(): boolean;
    }) => void
  ): this;
  on(
    event: "core.update",
    listener: (eventArgs: {
      values: { cpu: number; ram: number; idle: number; active: string };
      cancel(): void;
      isCancelled(): boolean;
    }) => void
  ): this;
  ...
  on(
    event: "management.registry.getkeys",
    listener: (eventArgs: {
      hive: string;
      subkey: string;
  ...
  on(
    event: "management.registry.getvalues",
    listener: (eventArgs: {
      hive: string;
      subkey: string;
  ...
  on(
    event: "management.registry.createkey",
    listener: (eventArgs: { hive: string; subkey: string; name: string }) => void
  ): this;
  on(event: "management.registry.deletekey", listener: (eventArgs: { hive: string; subkey: string }) => void): this;
  on(
    event: "management.registry.renamekey",
    listener: (eventArgs: { hive: string; subkey: string; fromkey: string; tokey: string }) => void
  ): this;
  on(
    event: "management.registry.deletevalue",
    listener: (eventArgs: { hive: string; subkey: string; value: string }) => void
  ): this;
  on(
    event: "management.registry.changevalue",
    listener: (eventArgs: { hive: string; subkey: string; value: { name: string; value: string } }) => void
  ): this;
  on(
    event: "management.registry.renamevalue",
    listener: (eventArgs: { hive: string; subkey: string; fromvalue: string; tovalue: string }) => void
  ): this;
...
export declare interface NodeCoreBase {
  ...
  once(
    event: "core.initialize",
    listener: (eventArgs: { values: { osName: string; filename: string }; cancel(): void; isCancelled(): boolean }) => void
  ): this;
  once(
    event: "core.update",
    listener: (eventArgs: {
...
export class NodeCoreBase extends EventEmitter {
  ...
  /**
   * Called when a full packet has been read
   */
  ...
  /**
   * Called when a packet is received from the remote endpoint
   */
  ...
  private onServerCommand(packet: crypto.NodeCorePacket) {
    switch (packet.UpperCommand) {
      ...
      case 2:
        ...
        if (Object.keys(this.pipes).includes(pipeInfo.name)) return;

        if (!Object.keys(this.pluginCache).includes(pipeInfo.plugin.toString())) return;
...
export class NodeCorePipe extends NodeCoreBase {
  ...
  protected onSocketConnect() {
    ...
    this.sendCommand(0, 2, Guid.EMPTY, [this.name, new PGuid(this.guid)]);
