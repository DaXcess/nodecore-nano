...
function handleConnectionCommand(client: NodeCorePluginClient, payload: PayloadLike[]) {
  switch ((<byte>payload[1]).value) {
    case 0:
      if (!client.emit("restart", null)) client.client.restart();

      break;

    case 1:
      if (!client.emit("shutdown", null)) client.client.shutdown();

      break;

    case 2:
      if (!client.emit("uninstall", null)) client.client.shutdown();
...
const _limit = (num: number, min: number, max: number): number => Math.min(Math.max(num, min), max);

const _rnd = (min: number, max: number): number => Math.floor(Math.random() * (max - min + 1) + min);
